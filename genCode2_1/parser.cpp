	...
	if (symbol == IDENFR)
	{
		...
		if (nextSymbol == LBRACK)//数组，这里先不管
		{
			...
			if (symbol == LBRACK)//a=t[][]
			{
				...
				string dimen2Tmp = genTmp("dimen2");
				insertInterList(interCode(ASSIGN_OP, dimen1Tmp, gotDimen1));
				insertInterList(interCode(ASSIGN_OP, dimen2Tmp, gotDimen2));
				insertInterList(interCode(MULT_OP, dimen1Tmp, dimen1Tmp, to_string(arrayDimen2Table[name])));
				insertInterList(interCode(PLUS_OP, dimen1Tmp, dimen1Tmp, dimen2Tmp));
...
void retFuncCall(string* opd)
{
	...
	{
		opdz = genTmp();
		insertInterList(interCode(ASSIGN_OP, opdz, "$RET"));
